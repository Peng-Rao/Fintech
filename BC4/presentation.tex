...
\begin{frame}{Three deep novelty detectors}
	...
	\begin{itemize}
		...
		\item \textbf{LSTM autoencoder} -- \alert{bidirectional 2-layer} encoder over a 12-week rolling window; scored by an \alert{ensemble of last-step reconstruction MSE and Mahalanobis distance} in the bottleneck latent.
	\end{itemize}
\end{frame}

\begin{frame}{Visual evidence: do anomalous weeks actually separate?}
	\vspace{-0.45em}
	\begin{columns}[T,onlytextwidth]
		\column{0.68\textwidth}
		\centering
		\includegraphics[width=\textwidth,height=0.76\textheight,keepaspectratio]{figures/14_latent_space_dim_reduction.pdf}

		\column{0.28\textwidth}
		\vspace{0.8em}
		\small
		\textbf{Raw features}\\[-0.1em]
		\scriptsize Partial separation only.

		\vspace{1.2em}
		\small
		\textbf{VAE latent}\\[-0.1em]
		\scriptsize Anomalies stay near the normal manifold.

		\vspace{1.2em}
		\small
		\textcolor{accent}{\textbf{LSTM bottleneck}}\\[-0.1em]
		\scriptsize Clear t-SNE/UMAP cluster; PACMAP places anomalies on the manifold perimeter.
	\end{columns}
\end{frame}
...
\begin{frame}{Out-of-sample detector results}
	\vspace{-0.35em}
	\metriccard{Best recall}{86.6\%}{LSTM autoencoder}
	\hfill
	\metriccard{Lowest missed-crisis rate}{13.4\%}{LSTM autoencoder}
	\hfill
	\metriccard{Best PR-AUC}{83.5\%}{LSTM autoencoder}
	\hfill
	\metriccard{Best F2}{0.820}{LSTM autoencoder}
	...
	\begin{columns}[T,onlytextwidth]
		...
		\begin{tabular}{lrrrr}
			...
			\midrule
			LSTM autoencoder   & \textbf{86.6\%} & \textbf{13.4\%} & \textbf{83.5\%} & \textbf{67.8\%} \\
			VAE reconstruction & 80.7\%          & 19.3\%          & 79.5\%          & 67.1\%          \\
			GAN reconstruction & 74.0\%          & 26.1\%          & 79.7\%          & 67.7\%          \\
		...
		LSTM sweeps every metric: highest recall, lowest missed-crisis rate, best PR-AUC, and best precision -- the bidirectional encoder + Mahalanobis latent score is doing the work.
	\end{columns}
\end{frame}
...
\begin{frame}{Portfolio impact: drawdown reduction}
	\begin{columns}[T,onlytextwidth]
		...
		\begin{tabular}{lrr}
			...
			GAN reconstruction & 7.53          & -18.7\%          \\
			VAE reconstruction & 5.21          & -17.6\%          \\
			LSTM autoencoder   & \textbf{8.32} & \textbf{-12.3\%} \\
			\bottomrule
		\end{tabular}
		...
		\small
		\begin{itemize}
			\item \textbf{LSTM dominates}: best wealth (8.32) \emph{and} best drawdown (-12.3\%).
			\item Beats buy \& hold on both axes simultaneously.
		\end{itemize}
	\end{columns}
\end{frame}
